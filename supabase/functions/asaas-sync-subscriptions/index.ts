...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!scheduled) {
      ...
      const { data: authData, error } = await admin.auth.getUser(authHeader.slice(7));
    ...
    // Item-level failures are part of the audit report, not a transport error.
    // Returning 200 lets the admin UI display the actual Asaas/database reason.
    return json({
      ok: failed === 0,
