...
-- Remove every stale overload before publishing the one canonical PostgREST contract.
-- A signature-specific DROP is insufficient when an interrupted deploy left an older overload behind.
DO $$
DECLARE stale_function RECORD;
BEGIN
  FOR stale_function IN
    SELECT p.oid::regprocedure AS signature
    FROM pg_catalog.pg_proc p
    JOIN pg_catalog.pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'pay_expense'
  LOOP
