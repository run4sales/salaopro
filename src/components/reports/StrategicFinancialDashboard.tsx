...
export function StrategicFinancialDashboard({ establishmentId, startDate, endDate }: Props) {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [cashRes, salesRes, futureAppointmentsRes, clientsRes, servicesRes, profsRes, apptServicesRes, apptProfsRes] = await Promise.all([
        supabase.from("cash_flow_entries").select("id, entry_type, amount, status, entry_date, source, payment_method, category, description").eq("establishment_id", establishmentId).is("deleted_at", null).gte("entry_date", comparisonStart).order("entry_date", { ascending: true }),
        supabase.from("sales").select("id, client_id, service_id, professional_id, amount, sale_date, payment_method, appointment_id").eq("establishment_id", establishmentId).is("deleted_at", null).gte("sale_date", comparisonStart).order("sale_date", { ascending: true }),
        supabase.from("appointments").select("id, client_id, service_id, professional_id, appointment_date, status").eq("establishment_id", establishmentId).gte("appointment_date", now.toISOString()).order("appointment_date", { ascending: true }).limit(1000),
        ...
        supabase.from("professionals").select("id, name, commission_percentage").eq("establishment_id", establishmentId),
        supabase.from("appointment_services").select("appointment_id, service_id").eq("establishment_id", establishmentId),
        supabase.from("appointment_professionals").select("appointment_id, professional_id").eq("establishment_id", establishmentId),
      ]);
      if (cashRes.error) throw cashRes.error; if (salesRes.error) throw salesRes.error; if (futureAppointmentsRes.error) throw futureAppointmentsRes.error;
      const clients = new Map((clientsRes.data ?? []).map((c: any) => [c.id, c.name]));
      const services = new Map((servicesRes.data ?? []).map((s: any) => [s.id, s]));
      const profs = new Map((profsRes.data ?? []).map((p: any) => [p.id, p]));
      const serviceIdsByAppt = new Map<string, string[]>();
      for (const row of apptServicesRes.data ?? []) serviceIdsByAppt.set((row as any).appointment_id, [...(serviceIdsByAppt.get((row as any).appointment_id) ?? []), (row as any).service_id]);
      const profIdsByAppt = new Map<string, string[]>();
      for (const row of apptProfsRes.data ?? []) profIdsByAppt.set((row as any).appointment_id, [...(profIdsByAppt.get((row as any).appointment_id) ?? []), (row as any).professional_id]);

      const cash = (cashRes.data ?? []).filter((r: any) => new Date(r.entry_date) <= endDate);
      ...
      const futureExpenses = (cashRes.data ?? []).filter((r: any) => r.entry_type === "expense" && new Date(r.entry_date) > now);
      const futureReceivables = (cashRes.data ?? []).filter((r: any) => r.entry_type === "income" && r.status === "pending" && new Date(r.entry_date) > now);
      const sales = (salesRes.data ?? []).filter((s: any) => new Date(s.sale_date) >= startDate && new Date(s.sale_date) <= endDate);
      ...
      const forecastAppointments = (futureAppointmentsRes.data ?? []).filter((a: any) => {
