  ...
  const fetchProfile = async (userId: string) => {
    try {
      ...
      if (linkedProfileError) {
        console.warn('Could not load linked establishment profile:', linkedProfileError);
      }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        ...
        if (session?.user) {
          ...
          // but keep protected screens loading until employee role/professional context is ready.
          setTimeout(async () => {
