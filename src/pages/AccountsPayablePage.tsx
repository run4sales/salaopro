...
export default function Expenses() {
  ...
  const changeForm = (field: Exclude<keyof FormState, "recurring">, value: string) => setForm((current) => ({ ...current, [field]: value }));
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(editing ? "Despesa atualizada." : form.recurring ? "Conta recorrente mensal criada." : "Conta a pagar criada."); setEditing(null); setForm(initialForm()); invalidate();
    } catch (error: any) { toast.error(error.message); } finally { setBusy(false); }
  };
  // Editing always targets a single occurrence: recurring plans are never rewritten from here.
  const openEdit = (expense: Expense) => { setEditing(expense); setForm({ description: expense.description, amount: String(expense.amount), category: expense.category || "Outros", supplier: expense.supplier || "", cost_center: expense.cost_center || "Administrativo", notes: expense.notes || "", due_date: expense.due_date, competence_date: expense.competence_date || expense.due_date, installments: String(expense.installment_count || 1), occurrences: "12", recurring: false }); };
  ...
  const pay = async () => {
    ...
    setBusy(true);
    try {
      const { data, error } = await (supabase as any).rpc("pay_expense", {
  ...
  return <div className="min-h-screen bg-background"><header className="border-b bg-card"><div className="container mx-auto px-4 py-6"><h1 className="text-2xl font-bold">Contas a Pagar</h1><p className="text-muted-foreground">Do previsto à quitação, com integração automática ao fluxo de caixa</p></div></header><main className="container mx-auto px-4 py-6 space-y-5">
    ...
    {canManage && <Card><CardHeader><CardTitle className="flex gap-2"><Plus className="h-5 w-5" />{editing ? "Editar despesa" : "Nova conta a pagar"}</CardTitle></CardHeader><CardContent><form onSubmit={save} className="grid md:grid-cols-4 gap-3"><div className="md:col-span-2"><FormField label="Descrição" field="description" form={form} onChange={changeForm} /></div><FormField label="Fornecedor" field="supplier" form={form} onChange={changeForm} /><FormField label="Valor total" field="amount" type="number" form={form} onChange={changeForm} /><div className="space-y-1"><Label>Categoria</Label><Select value={form.category} onValueChange={(value) => changeForm("category", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{CATEGORIES.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></div><div className="space-y-1"><Label>Centro de custo</Label><Select value={form.cost_center} onValueChange={(value) => changeForm("cost_center", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{CENTERS.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></div><FormField label="Competência" field="competence_date" type="date" form={form} onChange={changeForm} /><FormField label="Vencimento" field="due_date" type="date" form={form} onChange={changeForm} />{!editing && !form.recurring && <FormField label="Parcelas" field="installments" type="number" form={form} onChange={changeForm} />}{!editing && <div className="md:col-span-2 rounded-md border p-3 flex items-center justify-between gap-4"><div><Label htmlFor="recurring-payable">Repetir todos os meses</Label><p className="text-xs text-muted-foreground mt-1">Cria esta conta mensalmente a partir do vencimento informado.</p></div><Switch id="recurring-payable" checked={form.recurring} onCheckedChange={(recurring) => setForm((current) => ({ ...current, recurring, installments: "1" }))} /></div>}{!editing && form.recurring && <FormField label="Qtd. de ocorrências" field="occurrences" type="number" form={form} onChange={changeForm} />}<div className="md:col-span-3 space-y-1"><Label>Observações</Label><Textarea value={form.notes} onChange={(event) => changeForm("notes", event.target.value)} /></div><div className="md:col-span-4 flex gap-2"><Button disabled={busy}>{busy ? "Salvando..." : "Salvar"}</Button>{editing && <Button type="button" variant="outline" onClick={() => { setEditing(null); setForm(initialForm()); }}>Cancelar</Button>}</div></form></CardContent></Card>}
    <Card><CardHeader><CardTitle className="capitalize">Despesas de {periodLabel}</CardTitle></CardHeader><CardContent className="space-y-4"><div className="flex flex-wrap items-end gap-2"><div className="space-y-1"><Label>Data inicial</Label><Input type="date" value={filters.from} onChange={(event) => setFilters({ ...filters, from: event.target.value })} /></div><div className="space-y-1"><Label>Data final</Label><Input type="date" value={filters.to} onChange={(event) => setFilters({ ...filters, to: event.target.value })} /></div><Button type="button" variant="outline" size="sm" onClick={() => shiftMonth(-1)}>Mês anterior</Button><Button type="button" variant="outline" size="sm" onClick={() => setFilters({ ...filters, from: format(startOfMonth(new Date()), "yyyy-MM-dd"), to: format(endOfMonth(new Date()), "yyyy-MM-dd") })}>Mês atual</Button><Button type="button" variant="outline" size="sm" onClick={() => shiftMonth(1)}>Próximo mês</Button><Button type="button" variant="ghost" size="sm" onClick={() => setFilters({ ...filters, from: "", to: "" })}>Todos os períodos</Button></div><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-2"><div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pl-9" placeholder="Descrição ou fornecedor" value={filters.q} onChange={(event) => setFilters({ ...filters, q: event.target.value })} /></div><Select value={filters.status} onValueChange={(status) => setFilters({ ...filters, status })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Todos os status</SelectItem>{Object.entries(STATUS).filter(([key]) => key !== "confirmed").map(([key, value]) => <SelectItem value={key} key={key}>{value[0]}</SelectItem>)}</SelectContent></Select><Select value={filters.category} onValueChange={(category) => setFilters({ ...filters, category })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Todas as categorias</SelectItem>{CATEGORIES.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select><Input placeholder="Filtrar fornecedor" value={filters.supplier} onChange={(event) => setFilters({ ...filters, supplier: event.target.value })} /><Input type="number" placeholder="Valor mínimo" value={filters.min} onChange={(event) => setFilters({ ...filters, min: event.target.value })} /><Input type="number" placeholder="Valor máximo" value={filters.max} onChange={(event) => setFilters({ ...filters, max: event.target.value })} /></div>
