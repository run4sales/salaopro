...
Foram identificados **12 achados**: **1 crítico, 1 alto, 5 médios, 3 baixos e 2 informativos**. O achado crítico e o alto foram corrigidos no código; os demais permanecem abertos ou parcialmente mitigados por dependerem de produto, plataforma ou validação dinâmica.
...
- **Severidade/status/confiança:** média; confirmado, aberto; alta.
...
- **Descrição:** payload financeiro bruto é persistido e dados de cliente são impressos no console do browser. Retenção, criptografia e acesso operacional não estão definidos no repositório.
- **Impacto:** exposição secundária de PII/financeiro e aumento de escopo de incidente.
- **Probabilidade:** média.
- **Recomendação:** allowlist/redação antes de logar, remover logs de dados do browser, definir retenção e acesso, registrar apenas IDs/códigos/correlation ID.
...
- **Descrição:** não há timestamp/nonce assinado ou tabela de evento único. Um evento autenticado antigo pode ser reenviado e eventos fora de ordem podem sobrescrever estado novo.
...
- **Recomendação:** armazenar ID do evento com `UNIQUE`, comparar timestamps/versões, tornar máquina de estados monotônica e reconciliar periodicamente com Asaas.
...
- **Severidade/status/confiança:** baixa; confirmado, aberto; alta.
- **Categoria:** CWE-521, CWE-620.
- **Local/componente:** `create-staff-user` e `update-staff-user`.
- **Descrição:** mínimo local de seis caracteres; owner/admin pode definir diretamente senha/e-mail do subordinado sem convite, confirmação do e-mail, MFA ou reautenticação recente.
...
- **Severidade/status/confiança:** baixa; melhoria preventiva aberta; alta.
- **Categoria:** CWE-942.
- **Local/componente:** headers de Edge Functions, inclusive criação/alteração de funcionário.
- **Descrição:** `Access-Control-Allow-Origin: *`. O bearer token ainda é necessário e não há cookie automático, portanto não foi confirmado bypass direto.
- **Impacto:** amplia superfícies de phishing/exfiltração caso token seja fornecido a uma origem maliciosa.
- **Probabilidade:** baixa.
- **Recomendação:** allowlist por ambiente, `Vary: Origin`, métodos/headers mínimos; manter webhook sem credenciais CORS e não depender de CORS como autenticação.
...
1. Idempotência/ordenação do webhook; rate limit/CAPTCHA em auth, booking e integrações.
2. Remover/redigir logs com PII e estabelecer retenção.
...
| `npm run test:security` | regressão AppSec | passou: 2/2 testes | nenhum |
